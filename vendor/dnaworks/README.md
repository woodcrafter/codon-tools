...
This repository now includes a lightweight web interface for running DNAWorks
inside Docker.
...
- edit the `DNAWORKS.inp` content
- submit a run with one click
- watch real-time logs
- download generated `LOGFILE.txt`

Build and run with plain Docker:

```
docker build -t dnaworks-web:latest .
docker run --rm -p 8080:8080 dnaworks-web:latest
```
